...
import { addExerciseToWorkout, createWorkout, removeExerciseFromWorkout } from "@/app/lib/workouts/actions";
import { getMuscleGroups, getExercisesByMuscleGroup, getRecentExercises } from "@/app/lib/workouts/sidebar-actions";
import { getTodayWorkout } from "@/app/lib/workouts/today-actions";
...
export default function WorkoutSidebar({ isOpen, onClose }: Props) {
    ...
    const addMovement = async (exerciseId: number) => {
        let wId = workoutId;
        if (!wId) { wId = await createWorkout(today); setWorkoutId(wId); }
    ...
    return (
        <>
            <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
                <div className="w-72 h-full bg-stone-50 border-l-2 border-amber-600 shadow-lg overflow-y-auto" onClick={e => e.stopPropagation()}>
                    {/* Header */}
                    ...
                    {loading ? (
                        ...
                        exercises.map(ex => (
                            <div key={ex.id} className="flex items-center border-b border-black/5">
                                <button type="button" onClick={() => { onClose(); router.push(`/workouts/${today}/${ex.exerciseId}`); }}
                    ...
                    {/* Go to today */}
                    <button type="button" onClick={() => { onClose(); router.push(`/workouts/${today}`); }}
                    ...
                    <div className="mt-auto border-t border-black/10 px-3 py-2">
                        <Link href="/books" onClick={onClose} className="flex items-center gap-1 text-black/40 hover:text-black text-sm transition-colors">
