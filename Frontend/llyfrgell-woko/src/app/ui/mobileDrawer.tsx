...
const MobileDrawer: React.FC<Props> = ({ isOpen, onClose }) => {
  return (
    <div
      className={`fixed flex flex-col justify-center items-center z-50 top-0 right-0 h-full w-full bg-white text-black transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
    >
      <button className="absolute right-4 top-4 p-3 text-black" onClick={onClose}>
        <FontAwesomeIcon className="text-3xl text-black" icon={faXmark}  />
      </button>
